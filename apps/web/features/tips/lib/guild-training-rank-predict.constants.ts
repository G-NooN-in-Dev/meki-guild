/** 입력 가능한 최대 길드 수 */
const TRAINING_PREDICT_MAX_GUILDS = 7

/** 예측에 필요한 최소 길드 수 (빈 칸 제외) */
const TRAINING_PREDICT_MIN_GUILDS = 2

/**
 * 길드 인덱스별 행 배경색 (pastel 토큰).
 * 기본은 /50, hover 시 투명도 없이 같은 톤을 씁니다.
 */
const TRAINING_PREDICT_GUILD_ROW_CLASS = [
...
function getTrainingPredictGuildRowClass(guildIndex: number): string {
	return TRAINING_PREDICT_GUILD_ROW_CLASS[guildIndex] ?? TRAINING_PREDICT_GUILD_ROW_CLASS[0]
}

export {
	getTrainingPredictGuildRowClass,
	TRAINING_PREDICT_GUILD_ROW_CLASS,
