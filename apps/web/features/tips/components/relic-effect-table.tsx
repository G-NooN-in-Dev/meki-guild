...
/**
 * 유물별 장착·보유 효과 표.
 * 행마다 각성 단계를 바꿔 두 효과를 같이 비교하고, 잠재옵션은 Dialog로 엽니다.
 */
			...
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div className="flex min-w-0 flex-col gap-1">
					...
					<p className="text-grayscale-600 text-sm">
						각성 단계를 변경하면 장착 효과와 보유 효과가 함께 변경됩니다. 보유 효과는 장착하지 않아도 적용됩니다.
					</p>
				</div>
				<RelicPotentialTable />
			</div>
				...
				<Table className="w-full min-w-3xl">
					...
					<TableBody>
						{RELICS.map((relic) => {
							...
							const possessionLines = resolveRelicPossessionLines(relic.id, stage)
							...
							return (
								<TableRow key={relic.id} className="border-grayscale-200 hover:bg-transparent">
									...
									<TableCell className={possessionCellClassName}>
										{possessionLines.length ? (
											<ul className="text-grayscale-600 space-y-1 text-xs leading-snug break-keep sm:text-sm">
												{possessionLines.map((line) => (
													<li key={`${relic.id}-possession-${line}`}>{emphasizeEffectNumbers(line)}</li>
												))}
											</ul>
										) : (
											<p className="text-grayscale-400 text-sm">—</p>
