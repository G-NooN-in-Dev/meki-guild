...
import {
	RIVALRY_PREDICT_MAX_GUILDS,
	RIVALRY_PREDICT_MIN_GUILDS
} from '@/features/tips/lib/guild-rivalry-rank-predict.constants'
import { buildRivalryPredictResult, collectGuildNames } from '@/features/tips/lib/guild-rivalry-rank-predict.helpers'
import type {
	MgfGuildInfoResponse,
	RivalryPredictMember,
	RivalryPredictResult
} from '@/features/tips/types/guild-rivalry-rank-predict.type'
...
type PredictPhase =
	...
	| { type: 'sorting' }
	| { type: 'predicting' }
	| { type: 'done'; result: RivalryPredictResult }
	| { type: 'error'; message: string }

function createEmptyGuildInputs(): string[] {
	return Array.from({ length: RIVALRY_PREDICT_MAX_GUILDS }, () => '')
}
...
function GuildRivalryRankPredictBoard() {
	...
	async function handlePredict() {
		...
		if (names.length < RIVALRY_PREDICT_MIN_GUILDS) {
			setPhase({
				type: 'error',
				message: `길드명을 최소 ${RIVALRY_PREDICT_MIN_GUILDS}개 이상 입력해 주세요.`
			})
			return
		}
		...
		try {
			const allMembers: RivalryPredictMember[] = []
			...
			const result = buildRivalryPredictResult(allMembers)
				...
				<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{guildInputs.map((value, index) => (
						<div key={index} className="flex flex-col gap-1.5">
							<Label htmlFor={`rivalry-predict-guild-${index}`}>길드 {index + 1}</Label>
							<Input
								id={`rivalry-predict-guild-${index}`}
								value={value}
								disabled={isBusy}
								placeholder="길드명 입력"
								onChange={(event) => {
			...
			{phase.type === 'done' && (
				<div className="grid items-start gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(16rem,1fr)]">
					<div className="order-2 flex min-w-0 flex-col gap-3 lg:order-1">
						...
						<RivalryPredictMemberTable rows={phase.result.members} />
					</div>

					<aside className="order-1 flex min-w-0 flex-col gap-3 lg:sticky lg:top-24 lg:order-2">
						<h2 className="text-grayscale-900 text-base font-semibold">길드 대항전 예측 순위</h2>
						<RivalryPredictGuildTable rows={phase.result.guilds} />
					</aside>
				</div>
			)}
		</div>
